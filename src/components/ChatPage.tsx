...
export function ChatPage({ mentorId, chatId, onBack, onOpenChat, sidebarOpen }: ChatPageProps) {
  ...
  const handleSendMessage = async () => {
    ...
    if (!chatIdToUse && mentorId) {
      try {
        chatIdToUse = await createChat({ mentorId: mentorId as any });
  ...
  const currentMentor = chat?.mentor || mentor;
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {chat?.messages?.map((msg, index) => (
          <div
            key={index}
            className={`flex items-start gap-3 ${msg.isFromUser ? "justify-end" : "justify-start"}`}
          >
            {/* Profile Image - Left side for mentor, right side for user */}
            {!msg.isFromUser && (
        ...
        <div className="flex gap-2 items-end">
          <div className="flex-1">
            <textarea
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent resize-none min-h-[44px] max-h-[120px] overflow-y-auto"
              disabled={isLoading}
              rows={1}
              style={{ 
                scrollbarWidth: 'thin',
          ...
          <button
            ...
            className="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex-shrink-0"
          >
            Send
          </button>
        </div>
